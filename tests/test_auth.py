...
class TestAuthMiddleware:
    ...
    def test_malformed_bearer_only_denied(self):
        response = self.client.get("/api/v2/agents", headers={"Authorization": "Bearer"})
        assert response.status_code == 401
        assert "Invalid authorization header format" in response.text
    ...
    def test_valid_bearer_case_insensitive_allowed(self):
    ...
    def test_bearer_multiple_spaces_allowed(self):
        response = self.client.get("/api/v2/agents", headers={"Authorization": "Bearer    ao_user_valid"})
        assert response.status_code == 200
